...
import { marketTitle, selectionLabel } from '@/lib/markets';
import { colors, radius, spacing } from '@/lib/theme';
import type { Bet, BetStatus } from '@/types/db';

export const STATUS_META: Record<BetStatus, { label: string; color: string; bg: string; icon: 'time-outline' | 'checkmark-circle' | 'close-circle' | 'refresh-circle' }> = {
    ...
    <View style={styles.card}>
      ...
      <View style={styles.foot}>
        ...
        {bet.status === 'pending' ? (
          ...
          <Stat label="Ödeme" value={money(bet.payout ?? 0)} highlight={bet.status === 'won'} />
        )}
      </View>
    </View>
  );
